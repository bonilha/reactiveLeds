...
# • B0 (0xB0): config => bands/fps/signal_hold_ms
...
PKT_AUDIO_V2 = 0xA2
PKT_AUDIO    = 0xA1
PKT_CFG      = 0xB0  # [B0][ver u8][num_bands u16][fps u16][hold_ms u16][reserved u16]
LEN_A2 = 163
LEN_A1 = 161
LEN_CFG = 10
...
# Paleta e helpers (idem versões anteriores) ...
    ...
    try:
        ...
        while True:
            if select.select([sys.stdin], [], [], 0)[0]:
                ...
                if key == 'n': pending_key_change = 'next'
                elif key == 'p': pending_key_change = 'prev'
                elif key == 'q': os._exit(0)
    finally:
        termios.tcsetattr(sys.stdin, termios.TCSADRAIN, old_settings)

# UDP Receiver + CFG handler
latest_packet = None
...
def udp_receiver():
    ...
    global EXPECTED_BANDS, CFG_FPS, SIGNAL_HOLD_MS
        ...
        try:
            ...
            if hdr == PKT_AUDIO_V2 and n == LEN_A2:
                ...
                if fps > 0:
                    CFG_FPS = int(fps)
                if hold >= 0:
                    SIGNAL_HOLD_MS = int(hold)
                # status
                print(f"\n[CFG] bands={EXPECTED_BANDS} fps={CFG_FPS} hold={SIGNAL_HOLD_MS}ms -> aplicado no Pi")
            else:
...
def print_status(effect_name, ctx):
    ...
    print(f"\r{status:<70}", end="", flush=True); _last_status = now
